	...
	<form action="/user/authorize" id="userLoginForm" class="form-horizontal" method="post" accept-charset="utf-8">
		<fieldset>
			...
			<div class="form-actions">
				...
				<div class="submit">
					<input class="btn btn-primary" type="button" value="Login" onclick="$.ajax({ url: '/user/authorize', type: 'post', dataType: 'json', data: { email: document.getElementById('userEmail').value, password: document.getElementById('userPassword').value }, success: function(data) { if (data.error)  { alert(data.error); } else { window.location.reload(); } } })">
				</div>
			</div>
		</fieldset>
	</form>
